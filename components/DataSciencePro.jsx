    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <header id="home" className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,#dbeafe,#ffffff_36%,#eff6ff)]">
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-6 lg:py-28">
          <div><div className="inline-flex rounded-full border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-blue-700 shadow-sm">Dynamic MCQs • AI tutor • Progress dashboard</div><h1 className="mt-7 max-w-4xl text-5xl font-black tracking-tight text-slate-950 sm:text-6xl lg:text-7xl">Master Data Science & AI with a focused study hub.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Daily rotating MCQs, AI help, user dashboard, roadmap, resources and production-ready UI.</p><div className="mt-8 flex flex-wrap gap-4"><a href="#quiz-section" className="rounded-2xl bg-blue-600 px-7 py-4 font-black text-white shadow-xl shadow-blue-600/25 hover:bg-blue-700">Start Learning</a><a href="#dashboard" className="rounded-2xl border border-slate-300 bg-white px-7 py-4 font-black text-slate-800 shadow-sm hover:bg-slate-50">View Dashboard</a></div></div>
      ...
      <section id="theory" className="mx-auto max-w-7xl px-4 py-16 lg:px-6"><div className="mb-10 text-center"><p className="font-black uppercase tracking-widest text-blue-600">Study modules</p><h2 className="mt-2 text-4xl font-black text-slate-950">Core Theory & Concepts</h2></div><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">{chapters.map((chapter) => <article key={chapter.title} className="group rounded-3xl border bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex items-center justify-between"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 text-3xl">{chapter.icon}</div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">{chapter.tag}</span></div><h3 className="mt-5 text-xl font-black text-slate-950">{chapter.title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{chapter.text}</p><a href={chapter.link} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex font-black text-blue-600 group-hover:underline">Read Chapter →</a></article>)}</div></section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-6"><div className="rounded-[2rem] border bg-white p-8 shadow-sm"><h2 className="text-center text-4xl font-black text-slate-950">External Resources</h2><div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{resources.map(([label, href]) => <a key={label} href={href} target="_blank" rel="noopener noreferrer" className="rounded-2xl border bg-slate-50 p-5 font-black text-blue-600 hover:border-blue-500 hover:bg-blue-50">{label} ↗</a>)}</div></div></section>
